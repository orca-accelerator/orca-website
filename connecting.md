...
1. **Create a new key pair.**
   ...
   * After completion, two files will have been created: a **public key** and **private key**.
     ...
     Note: Your SSH key files (both the private key and the public key) must be placed in your `~/.ssh` directory before logging in. If they are not there, move them using the commands `mv keyname ~/.ssh/` and `mv keyname.pub ~/.ssh/`, replacing `keyname` with your actual key filenames.
